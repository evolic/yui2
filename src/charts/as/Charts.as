package
{
    ...
    import com.yahoo.astra.fl.charts.axes.*;
    import FireFlexLogger;
    import com.tomaszkuter.LogType;
    ...
    //--------------------------------------
        ...
        override protected function set component(value:DisplayObject):void
        {
            ...
            ExternalInterface.addCallback("getChartInfo", this.chart.getInfo);
    ...
    //--------------------------------------
        ...
        public function setType(value:String):void
        {
            if(this.chart)
            {
                ...
                this.chart.removeEventListener(ChartEvent.ITEM_DD_START, chartItemDDEventHandler);
                this.chart.removeEventListener(ChartEvent.ITEM_DD, chartItemDDEventHandler);
                this.chart.removeEventListener(ChartEvent.ITEM_DD_END, chartItemDDEventHandler);
            }
            ...
            this.chart.addEventListener(ChartEvent.ITEM_DD_START, chartItemDDEventHandler, false, 0, true);
            this.chart.addEventListener(ChartEvent.ITEM_DD, chartItemDDEventHandler, false, 0, true);
            this.chart.addEventListener(ChartEvent.ITEM_DD_END, chartItemDDEventHandler, false, 0, true);
         ...
         */
        public function getHorizontalAxis():Object
        {
            var cartesianChart:CartesianChart = this.chart as CartesianChart;
            if(cartesianChart)
            {
                return AxisSerializer.getAxis(cartesianChart.horizontalAxis);
            }

            var shortName:String = ChartSerializer.getShortName(getQualifiedClassName(this.chart));
            this.log("Unable to find horizontalAxis on a chart of type " + shortName, LoggerCategory.WARN);
            return null;
        }
         ...
         */
        public function getVerticalAxis():Object
        {
            var cartesianChart:CartesianChart = this.chart as CartesianChart;
            if(cartesianChart)
            {
                return AxisSerializer.getAxis(cartesianChart.verticalAxis);
            }

            var shortName:String = ChartSerializer.getShortName(getQualifiedClassName(this.chart));
            this.log("Unable to find verticalAxis on a chart of type " + shortName, LoggerCategory.WARN);
            return null;
        }
        ...
        public function calculateItemNewValue(seriesIndex:int, itemIndex:int, x:int, y:int):Number
        {
            var currentData:ISeries = this.chart.dataProvider[seriesIndex] as ISeries;
            var markerSize:Number = UIComponentUtil.getStyleValue(UIComponent(currentData), "markerSize") as Number;
            var chartInfo:Object = this.chart.getInfo();
            var xAxis:Object = this.getHorizontalAxis();
            var yAxis:Object = this.getVerticalAxis();

            var value:Number = 0;

            value += y;
            value -= chartInfo.contentBounds.y;
            value -= chartInfo.content.y;
            value -= chartInfo.contentPadding;
            value = yAxis.maximum - value/yAxis.positionMultiplier;
            var fixation:Number = yAxis.majorUnit/100;
            value = Math.round(value/fixation)*fixation;
        ...
        /**
         * Sets the JavaScript function to call to generate the chart's data tip for temporary value
         * during drag and drop opearation.
         */
        public function setTemporaryValueTipFunction(value:String):void
        {
            var delegate:Object = {temporaryValueTipFunction: JavaScriptUtil.createCallbackFunction(value).callback};
            delegate.callback = function(item:Object, index:int, series:ISeries, newValue:Number):String
            {
                return delegate.temporaryValueTipFunction(item, index, SeriesSerializer.writeSeries(series), newValue);
            }

            this.chart.temporaryValueTipFunction = delegate.callback;
        }
    ...
    //--------------------------------------
        ...
        override protected function initializeComponent():void
        {
            ...
            try
            {
                ...
                ExternalInterface.addCallback("setTemporaryValueTipFunction", setTemporaryValueTipFunction);
                ...
                ExternalInterface.addCallback("getHorizontalAxis", getHorizontalAxis);
                ExternalInterface.addCallback("setHorizontalAxis", setHorizontalAxis);
                ExternalInterface.addCallback("getVerticalAxis", getVerticalAxis);
        ...
        protected function chartItemExtraEventHandler(event:MouseEvent):void
        {
            ...
            if(renderer is ISeriesItemRenderer)
            {
                ...
                var currentData:ISeries = this.chart.dataProvider[seriesIndex] as ISeries;
                var newValue:Number = this.calculateItemNewValue(seriesIndex, itemIndex, this.mouseX, this.mouseY);
                var itemEvent:Object = {type: dragEventType, seriesIndex: seriesIndex, index: itemIndex, item: renderer.data, x: this.mouseX, y: this.mouseY, newValue: newValue};
                this.dispatchEventToJavaScript(itemEvent);
         ...
         * @private (protected)
         *
         * Receives chart item drag and drop events and passes them out to JavaScript.
         */
        protected function chartItemDDEventHandler(event:ChartEvent):void
        {
            var series:Series = event.series as Series;
            var renderer:ISeriesItemRenderer = series.lastMouseItem;
            var marker:DisplayObject = renderer as DisplayObject;
            var seriesIndex:int = (this.chart.dataProvider as Array).indexOf(series);
            var itemIndex:int = renderer.series.dataProvider.indexOf(renderer.data);
            ...
            var startingItemPosition:Point = series.startingItemPosition;
            var lastItemPosition:Point = series.lastItemPosition;
            var startingMousePosition:Point = series.startingMousePosition;
            var lastMousePosition:Point = series.lastMousePosition;

            var currentData:ISeries = series as ISeries;
            var markerSize:Number = UIComponentUtil.getStyleValue(UIComponent(currentData), "markerSize") as Number;
            var x:Number;
            var y:Number;
            ...
            var newValue:Number = this.calculateItemNewValue(seriesIndex, itemIndex, x, y);
            itemEvent.newValue = newValue;
            this.chart.temporaryValue = newValue;
