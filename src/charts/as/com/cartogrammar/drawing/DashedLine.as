 ...
 * @published May 2008
 * @link http://andywoodruff.com/blog/drawing-dashed-lines-with-actionscript-3/
 *
 * This is a UIComponent with the capability to do basic lineTo drawing with dashed lines.
 *
 * Example:
...
package com.cartogrammar.drawing {
    ...
    public class DashedLine extends UIComponent {

        private var lengthsArray:Array = new Array();    // array of dash and gap lengths (dash,gap,dash,gap....)

        public function lineLengthsStyle(lengthsArray : Array = null):void {
         ...
         * @private
         */
        public function setLineAlpha(value:Number):void
        {
            if (lineAlpha != value)
            {
                lineAlpha = value;
        ...
        private var startIndex:uint = 0;    // array index (the particular dash or gap) to start with in a lineTo--based on the last dash or gap drawn in the previous lineTo (along with remainingDist, this is so our line can properly continue around corners!)
        ...
        public function DashedLine(lineWeight:Number = 0, lineColor:uint = 0, lengthsArray:Array = null){
        ...
        public function lineTo(x:Number, y:Number):void {
            ...
            outerLoop : while (Math.abs(startX-curX) < Math.abs(startX-x) || Math.abs(startY-curY) < Math.abs(startY-y)){
                ...
                for (var i:uint = startIndex; i < lengthsArray.length; i++){
        ...
        public function lineStyle(lineWeight:Number = 0, lineColor:uint = 0, lineAlpha:Number = 1):void {
            stroke.graphics.lineStyle(lineWeight, lineColor, lineAlpha, false, "none", CapsStyle.NONE);
        }

        // basic beginFill
        public function beginFill(lineColor:uint , lineAlpha:Number = 1):void {
            fill.graphics.beginFill(lineColor, lineAlpha);
        }
        // basic endFill
