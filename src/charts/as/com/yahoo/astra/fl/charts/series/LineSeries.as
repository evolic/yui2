﻿package com.yahoo.astra.fl.charts.series
{
    ...
    //--------------------------------------
         ...
         * @private
         */
        private var _normalLine:Sprite;

        /**
         * @private
         */
        public function get normalLine():Sprite
        {
            return this._normalLine;
        }

        /**
         * @private
         */
        public function set normalLine(value:Sprite):void
        {
            if (this._normalLine != value)
            {
                this._normalLine = value;
            }
        }
         ...
         * @private
         */
        public function get dashedLine():DashedLine
        {
            return this._dashedLine;
        }

        /**
         * @private
         */
        public function set dashedLine(value:DashedLine):void
        {
            if (this._dashedLine != value)
            {
                this._dashedLine = value;
            }
        }
         ...
         * @private
         */
        private static var defaultDashedLineStyle:Array = [8,4,2,4];
    ...
    //--------------------------------------
        ...
        public function LineSeries(data:Object = null)
        {
            ...
            this.dashedLine = new DashedLine();
            this.addChild(this.dashedLine);

            this.normalLine = new Sprite();
    ...
    //--------------------------------------
         ...
         * @private
         */
        override protected function onMarkerDD(event:Event):void
        {
            super.onMarkerDD(event);
            this.drawTemporaryLines();
        }
            ...
            for(var i:int = 0; i < itemCount; i++)
            {
                ...
                if(markerValid)
                {
                    ...
                    if(lastValidPosition && !lastMarkerValid && connectDiscontinuousPoints)
                    {
                        ...
                        this.setTemporaryLineStyle(connectDiscontinuousPoints);

                        //draw a discontinuous line from the last valid position and the new valid position
                        GraphicsUtil.drawDashedLine(this.normalLine.graphics, lastValidPosition.x, lastValidPosition.y, xPosition, yPosition, discontinuousDashLength, discontinuousDashLength);
                    ...
                    else if(!lastValidPosition || (!lastMarkerValid && !connectDiscontinuousPoints))
                    {
                        ...
                        this.normalLine.graphics.moveTo(xPosition, yPosition);
                    }
                    else //current and last position are both valid
                    {
                        ...
                        {
                            this.normalLine.graphics.lineTo(xPosition, yPosition);
                        }
                        else
                        {
                            this.normalLine.graphics.moveTo(xPosition, yPosition);
        ...
        /**
         * Draws temporary, dashed lines connected to dragged point.
         */
        public function drawTemporaryLines():void
        {
            var markerSize:Number = this.getStyleValue("markerSize") as Number;
            ...
            for(var i:int = 0; i < itemCount; i++)
            {
                ...
                if (i == 0) {
                    this.dashedLine.moveTo(x, y);
                } else {
                    if (i == this.lastItemIndex) {
                        x = this.lastItemPosition.x + (this.lastMouseItem as DisplayObject).width/2;
                        y = this.fixMouseY(mouseY);
                    }
                    this.dashedLine.lineTo(x, y);
                }
            }
        }

        /**
         * Clears normal lines connected to dragged point.
         */
        public function clearNormalLines():void
        {
            this.normalLine.graphics.clear();
        }

        /**
         * Clears temporary, dashed lines connected to dragged point.
         */
        public function clearTemporaryLines():void
        {
        ...
        private function beginAreaFill(showAreaFill:Boolean, color:uint):void
        {
            ...
            this.normalLine.graphics.beginFill(color, areaFillAlpha);
        }
        ...
        private function setPrimaryLineStyle(forceColor:Boolean = false):void
        {
            ...
            {
                this.normalLine.graphics.lineStyle(0, 0, 0);
                return;
            }
            ...
            this.normalLine.graphics.lineStyle(lineWeight, fillColor, lineAlpha);
        }

        /**
         * @private
         * Sets the line style when connecting dragged points. The forceColor flag
         * will use the color even when the connectPoints style is set to false.
         * This is used primarily to allow connectDiscontinousPoints to work
         * when connectPoints is false.
         */
        private function setTemporaryLineStyle(forceColor:Boolean = false):void
        {
            var connectPoints:Boolean = this.getStyleValue("connectPoints") as Boolean;
            if(!connectPoints && !forceColor)
            {
                this.dashedLine.lineStyle(0, 0, 0);
                return;
            }

            var lineWeight:int = this.getStyleValue("lineWeight") as int;
            lineWeight = Math.round(lineWeight/2);
            lineWeight = lineWeight > 0 ? lineWeight : 1;
            var fillColor:uint = this.getStyleValue("lineColor") as uint;
            var lineAlpha:Number = this.getStyleValue("lineAlpha") as Number;
            this.dashedLine.lineStyle(lineWeight, fillColor, lineAlpha);
            this.dashedLine.lineLengthsStyle(defaultDashedLineStyle);
        }
        ...
        private function closeAreaFill(vertical:Boolean, origin:Number, firstValidPosition:Point, lastValidPosition:Point):void
        {
            ...
            if(vertical)
            {
                this.normalLine.graphics.lineTo(lastValidPosition.x, origin);
                this.normalLine.graphics.lineTo(firstValidPosition.x, origin);
                this.normalLine.graphics.lineTo(firstValidPosition.x, firstValidPosition.y);
            }
            else
            {
                this.normalLine.graphics.lineTo(origin, lastValidPosition.y);
                this.normalLine.graphics.lineTo(origin, firstValidPosition.y);
                this.normalLine.graphics.lineTo(firstValidPosition.x, firstValidPosition.y);
            }
            this.normalLine.graphics.endFill();
        }
